...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    ...
    if (request.cmd == 'update_font_size') {
      ...
      let dom2html = evalDOM({
        background: '#eee',
        init: function() {
          // 生成骨架屏之前的操作
  
          // 比如删除干扰节点
          let toTop = document.querySelector('#attach-title');
          if (toTop) {
      ...
      sendResponse('generate skeleton page success');
    } else {
  ...
  function evalDOM() {
    ...
    const classProps = {
      position: 'fixed',
      zIndex: 999,
      background: agrs.background || '#eee'
    };
    if (agrs.animation) {
      classProps.animation = agrs.animation;
    } else {
      classProps.animation = 'opacity 1s linear infinite reverse';
      classProps['animation-direction'] = 'alternate';
    }
    ...
    function createCommonClass(props) {
      ...
      inlineStyle.push(
        '}.__{top:0%;left:0%;width:100%;}@keyframes opacity {100%{opacity: 0;}}</style>'
      );
    ...
    DrawPageframe.prototype = {
      ...
      withHeader: function() {
        if (agrs.header) {
          ...
          const hBackground = background || agrs.background || '#eee';
          if (hHeight) {
            drawBlock({
